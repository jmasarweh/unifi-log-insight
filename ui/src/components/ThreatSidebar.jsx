...
function Section({ title, open, onToggle, children }) {
  return (
    <div className="border-t border-gray-800/50">
      <button
        onClick={onToggle}
        className="w-full flex items-center justify-between px-3 py-2 text-[11px] uppercase tracking-wider text-gray-400 hover:text-gray-300"
      >
        {title}
...
function Row({ label, value }) {
  ...
  return (
    <div className="flex justify-between items-baseline gap-2 py-0.5">
      ...
      <span className="text-gray-200 text-[12px] text-right truncate">{value}</span>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <span className="text-gray-200 text-[12px] truncate flex-1" title={remote}>
          {device}
        </span>
        <ThreatBadge score={log.threat_score} />
      </div>
      <div className="flex items-center justify-between gap-2 mt-0.5">
        <span className={`text-[10px] uppercase font-semibold ${ACTION_STYLES[log.rule_action] || 'text-gray-500'}`}>
        ...
        selectedLog ? (
          <div className="flex flex-col flex-1 min-h-0">
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-[11px] text-gray-400 hover:text-gray-200 border-b border-gray-800/50 shrink-0"
            >
